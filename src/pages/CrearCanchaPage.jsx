// src/pages/CrearCanchaPage.js
// Formulario para dar de alta un nuevo complejo.
...
export default function CrearCanchaPage() {
  ...
  const { agregarCancha } = useBooking();
  ...
  return (
    <KeyboardAvoidingView style={estilosComunes.pantalla} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <TouchableOpacity style={styles.imagePickerBox} activeOpacity={0.8} onPress={pickImage}>
          {imageUri ? <Image source={{ uri: imageUri }} style={styles.previewImage} /> : (
            <View style={styles.imagePlaceholder}>
              <Ionicons name="camera-outline" size={42} color={colores.acento} />
        ...
        <View style={styles.formSection}>
          ...
          <TextInput style={styles.input} placeholder="Ej. Predio San Francisco" placeholderTextColor={colores.textoInactivo} value={nombre} onChangeText={setNombre} />

          <Text style={styles.label}>Ubicación / Dirección</Text>
          <TextInput style={styles.input} placeholder="Ej. Av. Gutnisky 1200" placeholderTextColor={colores.textoInactivo} value={ubicacion} onChangeText={setUbicacion} />

          <View style={styles.row}>
            <View style={styles.halfColumn}>
              <Text style={styles.label}>Apertura</Text>
              <TextInput style={styles.input} placeholder="Ej. 14:00" placeholderTextColor={colores.textoInactivo} value={horaApertura} onChangeText={setHoraApertura} />
            </View>
            <View style={styles.halfColumn}>
              <Text style={styles.label}>Cierre</Text>
              <TextInput style={styles.input} placeholder="Ej. 23:00" placeholderTextColor={colores.textoInactivo} value={horaCierre} onChangeText={setHoraCierre} />
            </View>
          </View>

          <Text style={styles.label}>Precio por hora ($)</Text>
          <TextInput style={styles.input} placeholder="Ej. 12000" placeholderTextColor={colores.textoInactivo} keyboardType="numeric" value={precioHora} onChangeText={setPrecioHora} />

          <TouchableOpacity style={styles.submitBtn} activeOpacity={0.8} onPress={handleGuardar} disabled={isSubmitting}>
            {isSubmitting ? <ActivityIndicator color="#fff" /> : <Ionicons name="cloud-upload-outline" size={20} color={colores.textoPrimario} />}
